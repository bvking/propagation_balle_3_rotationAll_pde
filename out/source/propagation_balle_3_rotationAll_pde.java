/* autogenerated by Processing revision 1286 on 2023-06-21 */
...
//  ballManager.updateAndDraw(lastBallPosition); // CLASSE ballManager utilisé plus tard avec le sample des coordoonées x
 ...
 public void lockOscillatorToPositionFromPreviousProagedBallOriginal() { 
  ...
  if (key == 'R') {
   ...
   //   netPhaseBase[i] -= (i*TWO_PI/5)%PI/5;
    ...
    for (int i = 0; i < networkSize-0; i++) {
      ...
      netPhaseBase[i] -= (networkSize-1- oscillatorBlocked-i)*0.05f; // if oscillatorBlocked=0; net.phase[5] doesn't move
    //  netPhaseBase[i]=  netPhaseBase[i]%TWO_PI;
    ...
    for (int i = 0; i < networkSize; i++) {
      ...
      netPhaseBase[i] -=      (oscillatorBlocked+i)*0.05f;       //if oscillatorBlocked=0; net.phase[0] doesn't move
      netPhaseBase[i]=  netPhaseBase[i]%TWO_PI;
      key='#';
   ...
   if ( formerFormerKey == '#' || modeStartKeyToFollow == " null ") { // formerFormerKey == '#' || 
    ...
    println ( " modeStartKeyToFollow " + modeStartKeyToFollow);
     ...
     //   newPosFollowed[i]=-newPosFollowed[i];
      ...
      for (int i = 0; i < networkSize-0; i+=1) { 
         newPosFollowed[i]=map (0.08f, 0, 1, 0, TWO_PI); // signals to follow  
